...
from .viz import (plot_2d_trajectories, plot_3d_trajectories, animate_2d_trajectories, 
                  save_visualization, plot_interactive_2d, plot_4d_time_slider,
...
def run_deconfliction_check(primary: Flight, 
    ...
    if visualize:
        ...
        if interactive or time_slider or dashboard:
            try:
                if dashboard:
                    print("Generating interactive dashboard with manual controls...")
                    ...
                    if fig:
                        fig.show()
                        if output_file:
                            html_file = output_file.replace('.png', '_dashboard.html')
                            fig.write_html(html_file)
                            print(f"✓ Saved interactive dashboard to {html_file}")
                elif time_slider and include_z:
                    print("Generating 4D time-slider visualization...")
                    fig = plot_4d_time_slider(
                        primary, simulated, conflicts, safety_buffer
                    )
                    if fig:
                        fig.show()
                        if output_file:
                            html_file = output_file.replace('.png', '_4d.html')
                            fig.write_html(html_file)
                            print(f"✓ Saved interactive visualization to {html_file}")
                elif interactive:
                    print("Generating interactive 2D visualization...")
        ...
        if not interactive and not time_slider and not dashboard:
...
def main():
    ...
    parser.add_argument(
        '--animate',
        action='store_true',
        help='Generate animation (2D only, matplotlib)'
    )

    parser.add_argument(
        '--interactive',
        action='store_true',
        help='Generate interactive Plotly visualization (2D)'
    )
    ...
    parser.add_argument(
        ...
        action='store_true',
        dest='time_slider',
        help='Generate 4D time-slider visualization (3D + time, Plotly)'
    )

    parser.add_argument(
        '--dashboard',
        action='store_true',
        help='Generate interactive dashboard with manual controls (Plotly)'
    )

    parser.add_argument(
        '--output',
        type=str,
        help='Output file for visualization (PNG for static, HTML for interactive)'
    )
